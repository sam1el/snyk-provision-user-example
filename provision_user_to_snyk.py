...
class SnykAPIClient:
    ...
    def _make_request(
        ...
        # REST endpoints require a `version` query parameter. V1 endpoints do not.
        ...
        try:
            ...
            if self.debug:
                ...
                if headers_override:
                    headers_for_log.update(headers_override)
                if "Authorization" in headers_for_log:
                    headers_for_log["Authorization"] = "token [REDACTED]"
                logging.debug(f"Headers: {headers_for_log}")
                if json_data:
    ...
    def find_user_in_group(self, group_id: str, email: str) -> Optional[Dict]:
        ...
        # REST spec supports filtering memberships by user email. This avoids scanning/pagination and
        # makes the lookup far more reliable for large groups.
        params = {"limit": 100, "email": email}

        if self.debug:
            logging.debug(f"Searching for email: {search_email} in group {group_id} memberships (email filter)")

        response_data = self._make_request(
            "GET",
            endpoint,
            params=params,
            headers_override={"Accept": "application/vnd.api+json"},
        )

        if not response_data or "error" in response_data:
            if self.debug:
                logging.debug(
                    f"Failed to fetch group memberships: {response_data.get('error') if isinstance(response_data, dict) else 'Unknown error'}"
                )
            return None
        ...
        for membership in memberships:
            ...
            relationships = membership.get("relationships", {}) or {}
            user_data = (relationships.get("user") or {}).get("data", {})
            ...
            normalized_email = str(user_email).lower().strip() if user_email else ""
            if normalized_email != search_email:
                continue
            if not user_id:
                continue
        ...
        if self.debug:
            logging.debug(f"User {search_email} not found in group {group_id} memberships")
        return None
...
class UserProvisioner:
    ...
    def provision_single_user(self, org_id: str, email: str, role_public_id: str, group_id: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        is_already_exists = (
            ...
            or (
                status_code == 403
                and (
                    (result.get("code") == "84246666-373b-421a-9525-1ad740b3e787" and "already provisioned" in error_msg_lower)
                    or "already exists in the platform" in error_msg_lower
                    or "user already exists" in error_msg_lower
                    or "already provisioned" in error_msg_lower
        ...
        if is_already_exists:
            # If the user is already provisioned (pending first login), treat as success and stop.
            try:
                pending = self.client.list_pending_provisions(org_id)
            except Exception:
                pending = []
            ...
            for item in pending or []:
                ...
                pending_email = (
                    item.get("email")
                    or (item.get("user") or {}).get("email")
                    or (item.get("attributes") or {}).get("email")
                )
                if pending_email and str(pending_email).lower().strip() == search_email:
                    already = {
                        "success": True,
                        "email": email,
                        "org_id": org_id,
                    ...
                    logging.info(f"User {email} is already provisioned (pending first login) in org {org_id} - no action needed")
                    self.results["already_provisioned"].append(already)
                    time.sleep(0.2)
                    return already
            ...
            if not group_id:
                error_msg = (
                    "group_id is required when the user already exists in Snyk. "
                    "This script’s fallback workflow needs group membership to resolve the user_id before it can create an org membership."
                )
                logging.error(error_msg)
            ...
            if not user_info:
                error_msg = (
                    f"User {email} not found in group {group_id} memberships. "
                    "This usually means the user exists in Snyk but is not a member of this group yet, "
                    "or the API token cannot access group membership details. "
                    "In that case, you may need to invite/add the user to the group/org via the standard invite flow."
                )
                logging.error(error_msg)
                result["error"] = error_msg
    ...
    def provision_from_file(self, file_path: str, org_id: str, default_role_public_id: Optional[str] = None,
        ...
        return {
            ...
            "already_provisioned": len(self.results["already_provisioned"]),
    ...
    def get_results_summary(self) -> str:
        ...
        total_attempted = (
            len(self.results["provisioned"])
            + len(self.results["already_provisioned"])
            + len(self.results["org_memberships"])
            + len(self.results["already_added"])
        ...
        lines.append(f"Provisioned (Pending First Login): {len(self.results['provisioned'])}")
        lines.append(f"Already Provisioned (Pending): {len(self.results['already_provisioned'])}")
        ...
        if self.results["provisioned"]:
            lines.append("\n" + "-"*80)
            lines.append("PROVISIONED USERS (PENDING FIRST LOGIN):")
            lines.append("-"*80)
            lines.append("  Note: Provisioned users may not appear as active org members until they complete their first SSO login.")
            lines.append("")
        ...
        if self.results["already_provisioned"]:
            lines.append("\n" + "-"*80)
            lines.append("ALREADY PROVISIONED USERS (PENDING FIRST LOGIN):")
            lines.append("-"*80)
            for result in self.results["already_provisioned"]:
                lines.append(f"  ✓ {result['email']} → Org: {result['org_id']}")
                lines.append(f"    Status: Already provisioned (pending first login)")
                lines.append("")
        ...
        lines.append(
            ...
            f"{len(self.results['provisioned'])} provisioned + "
            f"{len(self.results['already_provisioned'])} already provisioned + "
            f"{len(self.results['org_memberships'])} org memberships + "
            f"{len(self.results['already_added'])} already added)"
        )
